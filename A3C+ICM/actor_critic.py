...
class ActorCritic(nn.Module):
    ...
    def calc_cost(self, new_state, hx, done, rewards, values, log_probs, intrinsic_reward=None):
        ...
        actor_loss = -T.sum(log_probs * gae)

        critic_loss = F.mse_loss(values[:-1].squeeze(), returns)
        # Note that if single state then values is rank 1 and returns rank 0 and need same shape
