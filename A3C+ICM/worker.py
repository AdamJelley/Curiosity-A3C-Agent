...
from A3C.utils import plot_learning_curve
from A3C.wrappers import make_env
import time
...
def worker(name, input_shape, n_actions, global_agent, optimizer, env_id, n_threads, global_idx, global_icm, icm_optimizer, icm):
    ...
    while episode < max_steps:
        ...
        while not done:
            ...
            memory.remember(obs, action, new_obs, reward, value, log_prob)
            score += reward
            obs = new_obs
            ep_steps += 1
            t_steps += 1
        ...
        if name == '1':
            ...
            print('A3C episode {} thread {} of {} steps {:.2f}M score {:.2f} avg score (100) {:.2f}'.format(episode, name, n_threads, t_steps/1e6, score, avg_score, avg_score_5000))
    if name == '1':
        x = list(range(episode))
        plot_learning_curve(scores, x, f'ICM_A3C_{env_id}.png')